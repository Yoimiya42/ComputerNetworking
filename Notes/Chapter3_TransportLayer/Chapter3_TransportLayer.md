...
### Multiplexing (Sender):
1. Each process passes message through its own socket ->
2. Transport-layer wraps each message into a **segment**, attaching a **(src port, dst port)** in the header ->
3. Each segment is passed to the network-layer, which encapsulates it into a **IP datagram** and sends it to the destination.
...
Different source IP/port but same destination IP/port -> delivered to the same single socket
...
4. process calls `recv()` to retrieve the data.
